   ...
   //   return $users;
    ...
    elseif ($id == 'ACAP')
      ...
      DB::table('student_login')->where('dte_id', $dte_id)->update(['acap_login' => 1]);
        ...
        //   $check = DB::select(DB::raw("select * from fees_transaction where dte_id LIKE '%".$dte_id."%' AND trans_status = 'Success' AND trans_amt = '1180' AND course = 'MEGACAP' "));
        //   if($check == null || $check == [] )
        ...
        //             DB::select("call insert_status_details_initiated('$dte_id','$course','$id')");
        //           }
        //           $registered = DB::table('mca_students')
        //           ->select("*")
        //           ->where('dte_id', $dte_id)
        //           ->exists();  
        //          if($registered == false ){             
        //             DB::table('mca_students')->insert([
        //                                       'dte_id' => $dte_id  
        //                                   ]);
        ...
        if($course == 'MEG')
        {
          ...
          $users = DB::select(DB::raw("SELECT event_to, status_to from status_details where dte_id LIKE '%".$dte_id."%' AND status_to = 'INITIATED' AND event_to LIKE '%".$id."%' ORDER BY updated_at DESC LIMIT 1"));
             ...
             $registered = DB::table('me_students')
            ->select("*")
            ->where('dte_id', $dte_id)
            ->exists();  
           if($registered == false ){             
              DB::table('me_students')->insert([
                                         'dte_id' => $dte_id  
                                    ]);
        ...
        {
        //   //CHANGE AMOUNT TO 1770
        //   $check = DB::select(DB::raw("select * from fees_transaction where dte_id LIKE '%".$dte_id."%' AND trans_status = 'Success' AND trans_amt = '1180' AND course = 'MCAACAP' "));
        //   if($check == null || $check == [] )
        ...
        {
        //   //CHANGE AMOUNT TO 1770
        //   $check = DB::select(DB::raw("select * from fees_transaction where dte_id LIKE '%".$dte_id."%' AND trans_status = 'Success' AND trans_amt = '1180' AND course = 'FEGACAP' "));
        //   if($check == null || $check == [] )
        ...
        //  }
         ...
         $registered = DB::table('fe_students')
            ->select("*")
            ->where('dte_id', $dte_id)
            ->exists();  
        ...
        else if($course == 'DSE')
        {
          $users = DB::select(DB::raw("SELECT event_to, status_to from status_details where dte_id LIKE '%".$dte_id."%' AND status_to = 'INITIATED' AND event_to LIKE '%".$id."%' ORDER BY updated_at DESC LIMIT 1"));
            ...
            $registered = DB::table('dse_students')
            ->select("*")
            ->where('dte_id', $dte_id)
            ->exists();  
           if($registered == false ){             
              DB::table('dse_students')->insert([
                                         'dte_id' => $dte_id  
                                    ]);
...
//     $request='user=VESINSTITUTE&password=123456&senderid=VESIPL&channel=Transactional&DCS=0&flashsms=0&number='.$mobile.'&text='.$message.'&route=4';
//     // $request1='http://bulksms.saakshisoftware.in/api/mt/SendSMS?user=VESINSTITUTE&password=123456&senderid=VESIPL&channel=transactionalDCS=0&flashsms=0&number=9769141254&text=testmessage&route=4';
//     // $request ='username=VESINSTITUTE&pass=123456&senderid=AVESIT&dest_mobileno='.$mobile.'&message='.$message.'&response=Y';
//   // $request='username=vivekanandeducation&pass=6_9mv!KS@&senderid=AVESIT&dest_mobileno='.$mobile.'&message='.$message.'&response=Y';
//     $ch = curl_init('http://bulksms.saakshisoftware.in/api/mt/SendSMS?');
...
//   }
  ...
  public function sendotp(Request $request)
    ...
    $request->session()->put('reg_mobile',$mobile);
    DB::select("call update_student_login_mobile('$dte_id','$mobile','$a')");
    // $request1='http://bulksms.saakshisoftware.in/api/mt/SendSMS?user=VESINSTITUTE&password=123456&senderid=VESIPL&channel=trans&DCS=0&flashsms=0&number=9769141254&text=test%20message&route=04';
    $request1="http://bulksms.saakshisoftware.in/api/mt/SendSMS?user=VESINSTITUTE&password=123456&senderid=VESIPL&channel=trans&DCS=0&flashsms=0&number=$mobile&text=$newmessage&route=04";
    // $request1='http://bulksms.saakshisoftware.in/api/mt/SendSMS?user=VESINSTITUTE&password=123456&senderid=VESIPL&channel=trans&DCS=0&flashsms=0&number=$mobile&text=$newmessage&route=4';
    $ch = curl_init();
      ...
      if ($dte_id != null)
      {
        ...
        if($user2==[] || $user2==null)
         ...
         elseif( $user2!=[])   
          ...
          $paymentdone =DB::select(DB::raw("SELECT * FROM `fees_transaction` WHERE admission_type LIKE 'ACAP' AND trans_status='SUCCESS' AND trans_amt='1' AND dte_id LIKE '%".$dte_id."%'"));
          if($paymentdone==[]){
              $paymentdone=null;
          }
     ...
     if($log_acap == null)
      ...
      if($log_dte == null){
        ...
        $data = array(
            'dte_id'=> $dte_id,
            'name'=> $name,
            'date'=>'from 27th Jan(11:59 P.M) to 28th Jan 2021 (4:30 P.M) ' 
          );
        Mail::send('user.acapmail', $data, function ($message) use($email)
          {
          $message->to($email, '')->subject('Payment of Against CAP Application fees');
          $message->from('vesit.admission@ves.ac.in', 'Vesit Admissions');
          });
      }      
...
public static function showdseAcademic(Request $request)
  ...
  public static function insertdseAcademic(Request $request)
  {
    ...
    if($diploma_university=='OTHER'){
          $diploma_university= $request->input('diplomaBScClgUniOther');
         }	
   ...
   public static function showMcaAcapFormPayment(Request $request)
    {
        ...
        $dte_id = $request->session()->get('log_dte_id');
        // DB::table('fees_transaction')->where('dte_id',$dte_id)->exists()
        //   $users = DB::table('fees_transaction')->where('dte_id',$dte_id)->get(); 
    ...
    if ($dte_id != 'null')
      {
          ...
          if ($dte_login[0]->dte_login == 1 || $acap_login[0]->acap_login == 1) 
          {
              ...
              $university_types_grad = array(
                'OMU' => 'OMU(Other than Mumbai University)',
                'OMS' => 'OMS(Other than Maharashtra State)',
                         ...
                         list( $user1['prog_val'],$userprogress)= HomeController::meprogressbar($course,$dte_id);
                            ...
                            if(DB::table('part_payment')->where('dte_id',$dte_id)->exists())
                            {
                             $user =DB::table('part_payment')->where('dte_id',$dte_id)->get();
                                ...
                                DB::table('admission')->where('dte_id',$dte_id)->where('course',$course)->update(['granted_amt' => $user[0]->amt],['balance_amt' => $user[0]->amt]);
                                   DB::table('admission')->where('dte_id',$dte_id)->where('course',$course)->update(['balance_amt' => $user[0]->amt]);
     ...
     if($log_acap == null)
      ...
      if($event=="ACAP"){
        ...
        $data = array(
            'dte_id'=> $dte_id,
            'name'=> $name,
            'date'=>'from 27th Jan(11:59 A.M) to 28th Jan (11:59 A.M) 2021 ' 
          );
     ...
     if($log_acap == null)
      ...
      if($log_dte == null)
        ...
        $data = array(
            'dte_id'=> $dte_id,
            'name'=> $name,
            'date'=>'from 29/01/2021 to 31/01/2021 ' 
          );
        Mail::send('user.acapmail', $data, function ($message) use($email)
          {
           $message->to($email, '')->subject('Payment of Against CAP Application fees');
