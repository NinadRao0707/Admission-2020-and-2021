 ...
 public static function updatevacantseats($id,Request $request)
    {
        ...
        elseif($id=="aids_plus")
        {
            $val=$user[0]->aids_vac + 1;
          DB::table('vacancy')->where('id',1)->update(['aids_vac'=>$val]);
        }
        elseif($id=="aids_plus_sindhi")
        {
            $val=$user[1]->aids_vac + 1;
          DB::table('vacancy')->where('id',2)->update(['aids_vac'=>$val]);
        }
        elseif($id=="aids_minus")
        {
            $val=$user[0]->aids_vac- 1;
          DB::table('vacancy')->where('id',1)->update(['aids_vac'=>$val]);
        }
        elseif($id=="aids_minus_sindhi")
        {
            $val=$user[1]->aids_vac- 1;
          DB::table('vacancy')->where('id',2)->update(['aids_vac'=>$val]);
        }
